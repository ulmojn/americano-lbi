...
import { Trophy, ArrowRight, Lock, Plus } from '@phosphor-icons/react';
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      <header className="border-b border-[#1A1A1A] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Trophy size={24} className="text-[#D1F441]" weight="fill" />
          <span className="font-display text-xl font-bold tracking-wider uppercase">Padel Pro</span>
        </div>
          ...
          <div className="space-y-3">
            {tournaments.map(t => (
              <Link
                key={t.id}
                to={`/tournament/${t.id}`}
                className="flex items-center justify-between p-4 bg-[#111] border border-[#1A1A1A] hover:border-[#D1F441]/30 hover:bg-[#141414] transition-all group"
              >
                <div>
